...
class AtasView(QWidget):
    def __init__(self, parent=None):
        ...
        self.change_db_location_action = self.db_menu.addAction(icon_manager.get_icon("database"), "Mudar Local do DB")
        self.db_menu.addSeparator()
        self.export_main_json_action = self.db_menu.addAction(icon_manager.get_icon("exportar"), "Exportar Dados Principais (JSON)")
        self.import_main_json_action = self.db_menu.addAction(icon_manager.get_icon("data-collection"), "Importar Dados Principais (JSON)")
        self.db_menu.addSeparator()
        self.export_complementary_json_action = self.db_menu.addAction(icon_manager.get_icon("external-link_azul"), "Exportar Dados Complementares (JSON)")
        self.import_complementary_json_action = self.db_menu.addAction(icon_manager.get_icon("data-collection2"), "Importar Dados Complementares (JSON)")
